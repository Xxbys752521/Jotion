...
const Navigation = () => {
  ...
  const siderebarRef = useRef<ElementRef<"aside">>(null);
  ...
  const handleMouseMove = (event: MouseEvent) => {
    if (isResizingRef.current) {
      ...
      if (siderebarRef.current && navbarRef.current) {
        siderebarRef.current.style.width = `${newWidth}px`;
  ...
  const resetWidth = () => {
    if (siderebarRef.current && navbarRef.current) {
      ...
      siderebarRef.current.style.width = isMobile ? "100%" : "240px";
      navbarRef.current.style.setProperty(
        "width",
  ...
  const collapse = () => {
    if (siderebarRef.current && navbarRef.current) {
      ...
      siderebarRef.current.style.setProperty("width", "0");
  ...
  return (
    <>
      ...
      <div
        ...
        onClick={resetWidth}
        className={cn(
          " absolute top-0 z-[9999] left-60 w-[calc(100%-160px)]",
          isResetting && "transition-all ease-in-out duration-300",
